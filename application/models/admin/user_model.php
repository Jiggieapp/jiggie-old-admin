...
class User_model extends CI_Model {
	...
	public function getAllUsers($arr_condition, $arr_sort, $return_type, $num = 0, $offset = 0, $arr_search=array()){
			...
			$this->db->where($arr_condition);
                ...
                if(!empty($arr_search["where"])) {
                    $this->db->where("(u.first_name  LIKE '%".$arr_search["where"]."%' OR 
                        ...
                        TIMESTAMPDIFF(YEAR, u.birthday, CURDATE()) LIKE  '".$arr_search["where"]."' OR 
		...
		if($num > 0){
		 ...
		 $this->db->select("u.fb_id as id ,CONCAT_WS(' ', u.first_name, u.last_name) as name, CONCAT_WS(' ', U1.first_name, U1.last_name) as inviteuser, u.profile_image_url , u.profile_image_url_2, u.profile_image_url_3, u.profile_image_url_4, u.profile_image_url_5, u.gender , IF(TIMESTAMPDIFF(YEAR, u.birthday, CURDATE()),  TIMESTAMPDIFF(YEAR, u.birthday, CURDATE()) ,  0 ) as age,
                                    
                                   u.location ,  u.invited_by ,DATE_FORMAT(u.created_at,'%m-%d-%Y %T') as usercreated_at , IF(u.verified = 1,  'Yes' ,  'No' ) as verified, 
    ...
    public function export($start_date, $end_date, $arr_sort,$arr_search=array()) {
        ...
        if(!empty($arr_search["where"])) { 
            $where_search = "AND (u.first_name  LIKE '%".$arr_search["where"]."%' OR 
                        u.last_name  LIKE '%".$arr_search["where"]."%' OR 
                        u.email  LIKE '%".$arr_search["where"]."%' OR 
                        CONCAT_WS(' ', u.first_name, u.last_name)  LIKE '%".$arr_search["where"]."%' OR
                        u.location  LIKE '%".$arr_search["where"]."%' OR 
        ...
        $query = $this->db->query($SQL="SELECT * FROM users u WHERE created_at >= '$start_date' AND created_at <= '$end_date' $where_search ".$order);
	...
	public function getInvitedBy($id=''){
		$this->db->where('id',$id);
    ...
    public function getUserCntByDate( $qry_condn){
        $result = 0;
        $query	= $this->db->query("Select count(id) as user_cnt from users where $qry_condn AND user_status =1");       
    ...
    public function getUserCount(){
        $query	= $this->db->query("Select count(id) as user_cnt from users WHERE user_status =1");
        foreach ($query->result_array() as $count){
            $result = $count['user_cnt'];
        }
    ...
    public function getUserHosting($id) {
		...
		$this->db->where("hosting_status!=", 10,FALSE);
    ...
    public function getUserChat($id) {
		...
		$this->db->or_where('host_id', $id); 
        $query = $this->db->from("conversation_topics");
        return $this->db->count_all_results();
    }
	
	public function getReturningUsersByDay($start,$end){
		 $query	= $this->db->query("SELECT date(l.time) as day,count(DISTINCT(l.user_id)) as ct 
		 FROM login_logs AS l LEFT JOIN `users` AS u ON `u`.`id` = `l`.`user_id`  WHERE l.time BETWEEN '$start' AND '$end' 
		 AND u.user_status  =1  AND  date(u.created_at) < date(l.time) GROUP BY date(l.time)  ORDER BY date(l.time) ASC ");
         $result =array();
         foreach ($query->result_array() as $count){
	...
	public function getReturningUsersByWeek($start,$end){
		 $query	= $this->db->query("SELECT date(l.time) as day,count(DISTINCT(l.user_id)) as count,  YEARWEEK(`l`.`time`,1) as week
		 FROM login_logs AS l LEFT JOIN `users` AS u ON `u`.`id` = `l`.`user_id`  WHERE l.time BETWEEN '$start' AND '$end' 
		 AND u.user_status  =1  AND  date(u.created_at) < date(l.time) GROUP BY week  ORDER BY day ");
		 $result =array();
         foreach ($query->result_array() as $count){
	...
	public function getReturningUsersByMonth($start,$end){
		$query	= $this->db->query("SELECT date(l.time) as day,count(DISTINCT(l.user_id)) as count, EXTRACT(YEAR_MONTH FROM `l`.`time`) as month
		 FROM login_logs AS l LEFT JOIN `users` AS u ON `u`.`id` = `l`.`user_id`  WHERE l.time BETWEEN '$start' AND '$end' 
		 AND u.user_status  =1  AND  date(u.created_at) < date(l.time) GROUP BY month  ORDER BY day ");
