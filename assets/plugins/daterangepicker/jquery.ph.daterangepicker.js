...
//				showCurrentAtPos: 1 // bug; use maxDate instead
	...
	function buildCalendar(classnameContext, options) {
		...
		function onSelectDay(dateText, instance) {
			...
			selectedDate 	=  moment(dateFormats(selectedDate1, "yyyy-mm-dd")+'T01:00:00-04:00').startOf('day').toDate();
			//sel_start 		= moment($('#startd').val()).startOf('day').toDate();
			//sel_end   		= moment($('#endd').val()).startOf('day').toDate();
			//var js = new TimeShift.OriginalDate( );
			//var je = new TimeShift.OriginalDate( );  	
			sel_start 		= moment(dateFormats($('#startd').val(), "yyyy-mm-dd")+'T01:00:00-04:00').startOf('day').toDate();
			sel_end   		= moment(dateFormats($('#endd').val(), "yyyy-mm-dd")+'T01:00:00-04:00').startOf('day').toDate();
	...
	function buildDateRangePicker($originalElement, options) {
		...
		function getStart(){
			...
			var edate  = moment(dateFormats($('#endd').val(), "yyyy-mm-dd")+'T01:00:00-04:00');
			var sdate  = moment(dateFormats($(this).val(), "yyyy-mm-dd")+'T01:00:00-04:00');
		...
		function getEnd(){
			...
			var edate 		= moment(dateFormats($(this).val(), "yyyy-mm-dd")+'T01:00:00-04:00');
			var sdate   		= moment(dateFormats($('#startd').val(), "yyyy-mm-dd")+'T01:00:00-04:00');
			re = /^\d{1,2}\/\d{1,2}\/\d{4}$/;
			var buttons = buttonPanel.getElement();
			$calendar = calendar.getRange();
